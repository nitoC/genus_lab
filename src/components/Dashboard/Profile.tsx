    ...
    const Chart: React.FC<ChartProps> = ({ data, color }) => (
        <div className="h-64">
            <svg className="w-full h-full" viewBox="0 0 500 100" preserveAspectRatio="none">
                <path
                    d={`M 0 ${100 - data[0]/1.2} ` + data.map((p, i) => `L ${i * (500 / (data.length -1))} ${100 - p/1.2}`).join(' ')}
                    fill="none"
                    stroke={color}
                    strokeWidth="2"
                />
                ...
                <path
                    d={`M 0 100 L 0 ${100 - data[0]/1.2} ` + data.map((p, i) => `L ${i * (500 / (data.length -1))} ${100 - p/1.2}`).join(' ') + ` L ${500} 100 Z`}
    ...
    return (
        <div className="p-4 sm:p-6 lg:p-8 text-white min-h-full">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-6 mb-8">
                <img src="https://i.pravatar.cc/100?img=3" alt="User Avatar" className="w-24 h-24 rounded-full border-4 border-blue-500"/>
                <div>
            ...
            <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                    <div className="bg-gradient-to-br from-blue-500 to-blue-700 p-6 rounded-2xl relative overflow-hidden">
                        <h4 className="text-lg font-semibold text-blue-100">Total Balance</h4>
                ...
                <div className=" p-6 rounded-2xl border border-gray-700">
                    <h4 className="text-lg font-semibold text-black">Earnings Over Time</h4>
